 ...
 * DoubleFetchChecker
 */
#include "XChecker/DoubleFetchChecker.h"
...
char DoubleFetchChecker::ID = 0;

static RegisterPass<DoubleFetchChecker> XCHECKER("DoubleFetchChecker", "X-Checker");

void DoubleFetchChecker::initSrcs()
{
    PAG* pag = getPAG();
...
void DoubleFetchChecker::initSnks()
{
    PAG* pag = getPAG();
...
bool DoubleFetchChecker::isInAWrapper(const SVFGNode* src, CallSiteSet& csIdSet)
{
    return false;
}


void DoubleFetchChecker::reportBug(ProgSlice* slice)
{
...
bool DoubleFetchChecker::isSourceLikeFun(const llvm::Function* func)
{
    if (!func)
        return false;
...
bool DoubleFetchChecker::isSinkLikeFun(const llvm::Function* func)
{
    if (!func)
        return false;
